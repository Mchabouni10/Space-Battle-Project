...
//class Spaceship=============================== 
    class Spaceship {
      ...
      attack(target) {
        ...
        if (accuracyShoot < this.accuracy) {  // statement if the accuracyshoot less than the set accuracy Alienship will be hit 
          ...
          if (target.hull < 0) { //statement if the target hull negative we bring it to zero to prevent negative value
            target.hull = 0;
          }
          ...
          if (target.hull <= 0) { // statement if there is no hull left mean the alienship is destroyed
            ...
            document.getElementById('alienHull').textContent = currentAlienShip.hull; // access to alienship shield 
          }
        } else { // statement that accuracyShoot is higher than the accuracy set mean we missed target
    ...
    class AlienShip {  
      constructor() {  //alienship propreties set to random using math floor and between certain variable that was given 
      ...
      attack(target) {  //function attack
        ...
        if (accuracyShoot < this.accuracy) {  // the same logic with attack target for spaceship, just in opposite side
    ...
    const alienShipImages = [ //set each alineship different shape and look for each alien
    ...
    function endRound() {
      if (ussShip.hull <= 0) { // if the spaceship hull less or zero mean is destoreyed 
        // that mean my attack and next round button are disabled because I lost. I only can restart the game
        console.log(`USS A-Ship is destroyed. Aliens win Round ${currentRound}! Game Over`);
        document.getElementById('attackButton').disabled = true;
        document.getElementById('nextRoundButton').disabled = true;
        document.getElementById('restartButton').disabled = false;
      } else if (currentAlienShip.hull <= 0) {
        console.log(`Alien ship is destroyed. You win Round ${currentRound}!`);
        document.getElementById('attackButton').disabled = true;
        // that mean alienship is destroyed already and i don't need to attack, i have to go to next round 

        if (currentRound < maxRounds) { //statement to check if I am not in the last round to disable next round button 
    ...
    document.getElementById('attackButton').addEventListener('click', function () {
      ...
      if (ussShip.hull > 0 && currentAlienShip.hull > 0) { // statement that both they can attack each other 
        ussShip.attack(currentAlienShip); // always spaceshup attack first 
        if (currentAlienShip.hull > 0) { // statement if the Alienship not destroyed by the first attack that cane attack back
